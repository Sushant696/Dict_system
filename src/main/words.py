...
word_list = [
    ...
    {"word": "Pernicious", "description": "having a harmful effect, especially in a gradual or subtle way"},
    {"word": "Ineffable", "description": "too great or extreme to be expressed or described in words"},
...
def display_words_ui(container_frame):
    for i, word_dict in enumerate(word_list):
        ...
        word_frame.grid(row=(i // 2) + 1 , column=i % 2, padx=70, pady=20, sticky="w")

        word_label = Label(word_frame, text=f"{i + 1}. {word_dict['word']}", font=("Helvetica", 12), justify="left", wraplength=400)
        word_label.grid(row=0, column=0, sticky="w")

        word_des_label = Label(word_frame, text=f"{word_dict['description']}", font=("Helvetica", 14), justify="left", wraplength=400)
